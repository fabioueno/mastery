defmodule Mastery.Boundary.TemplateValidator do
  ...
  def validate_instrutions(_instructions), do: {:error, "must be a binary"}
  ...
  def validate_generators(_genrators), do: {:error, "must be a map"}

  def validate_generator({name, generator}) when is_atom(name) and is_list(generator) do
    check(generator != [], {:error, "can't be empty"})
  end

  def validate_generator({name, generator}) when is_atom(name) and is_function(generator) do
    :ok
  end
  ...
  def validate_chcker(_checker), do: {:error, "must be an arity 2 function"}
end
